...
(
    ...
    externalContracts=(
        _base64/base64_inline_asm.sol
        _base64/base64_no_inline_asm.sol
        _prbmath/PRBMathSD59x18.sol
        _prbmath/PRBMathUD60x18.sol
    ...
    do
        ...
        then
            ...
            test_via_ir_equivalence "${REPO_ROOT}/test/libsolidity/semanticTests/externalContracts/${contractFile}"
        fi

        printTask " - ${contractFile} (optimized)"
        test_via_ir_equivalence "${REPO_ROOT}/test/libsolidity/semanticTests/externalContracts/${contractFile}" --optimize
    done
)
rm -r "$SOLTMPDIR"
